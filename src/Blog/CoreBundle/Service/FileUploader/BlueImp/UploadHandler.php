...
namespace Blog\CoreBundle\Service\FileUploader\BlueImp;
...
class UploadHandler
{
    ...
    function __construct($options=null) {
        ...
        if (isset($options['image_versions']) && !count($options['image_versions'])) {
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index) {
        ...
        $file->name = Urlizer::urlize($info['filename']) . '.' . $info['extension'];
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file->size = $file_size;
            $this->set_file_delete_url($file);
            if (isset($this->options['editId'])) {
                $file->serve_url = $this->options['router']->generate('serve_temp_file', array('editId' => $this->options['editId'], 'name' => $file->name, 'type' => $this->options['type']), UrlGeneratorInterface::ABSOLUTE_PATH);
            }

            // Carlos. 14/07/15. Allow delete original file
            if (isset($this->options['delete_original_file']) && $this->options['delete_original_file'] === true)
            {
                if (is_file($file_path)) {
                    unlink($file_path);
                    rmdir(substr($this->options['upload_dir'], 0, -1));
                }
            }
        }
        return $file;
    ...
    // Carlos 14/07/15. Redefine method
